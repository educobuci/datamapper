...
test('encoding keys', () => {
  const coder = new DataCoder<Product>({
    codingKeys: ['id', 'price', { name: 'localized_name' }],
  })
  const encoded = coder.encode({ id: 1, name: 'iPhone', price: 999 })
...
test('custom decoder', () => {
  ...
  const coder = new DataCoder<SearchableProduct>({
    decode: ({ id, name, price }) => ({
      id,
      name,
      price,

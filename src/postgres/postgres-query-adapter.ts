...
export class PostgresQueryAdapter {
  buildSqlQueryConfig(ast: QueryAST): QueryConfig {
    ...
    let text =
      `SELECT ${ast.select.columns.join(', ')} ` + `FROM ${ast.select.from}`
    let values = []
    if (ast.where) {
      if (ast.where._sql) {
        let sql = ast.where._sql[0] as string
        let index: number
        let count = 1
        ...
        text += ` WHERE ${sql}`
        values = (ast.where._sql as []).slice(1)
      } else {
    ...
    if (ast.order) {
      const order: string[] = Array.isArray(ast.order)
        ? ast.order
        : Object.entries(ast.order).map(([key, ordering]) =>
            ordering === 'asc' ? key : `${key} DESC`,
          )
  ...
  private buildInsertQuery(ast: QueryAST): QueryConfig {
    if (!ast.insert) {
      throw new Error('Insert query configuration is missing')
    }
    const text =
      `INSERT INTO ${ast.insert.into} ` +
  ...
  private buildUpdateQuery(ast: QueryAST): QueryConfig {
    if (!ast.update) {
      ...
      `WHERE ${
        ast.where
          ? Object.entries(ast.where)
              .map(
                ([key, _], i) =>
                  `${key} = $${i + Object.values(set).length + 1}`,
              )
              .join(' AND ')
          : ''
      } ` +
